...
moduleFor('adapter:gitlab-tree', 'Unit | Adapter | gitlab tree', {
...
test('it builds the URL correctly', function(assert) {
  ...
  assert.equal(adapter.buildURL('gitlab-tree', null, null, 'queryRecord', query), `${host}/repos/${repo}/git/trees/${sha}`);
});
